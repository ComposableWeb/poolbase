...
export default (ComposedComponent): NextPage<PropsWithAuthUserInfo> => {
  const WithAuthUserComp: NextPage<PropsWithAuthUserInfo> = (props: PropsWithAuthUserInfo): JSX.Element => {
  ...
  WithAuthUserComp.getInitialProps = async (ctx: NextPageContext): Promise<PropsWithAuthUserInfo> => {
    const { req, res } = ctx;
    ...
    set(ctx, 'poolbase.AuthUserInfo', AuthUserInfo);
